...
#    `functionName` is a function's name in your module (e.g., `fitBirdBiomassModelEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Users/Tati/Documents/GitHub/fitBirdBiomassModel/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("fitBirdBiomassModel")
  ...
  parameters <- list(
    ...
    fitBirdBiomassModel = list(.saveInitialTime = NA)
  )
  ...
  if (exists("fitBirdBiomassModelEvent1", envir = .GlobalEnv)) {
    simOutput <- fitBirdBiomassModelEvent1(mySim)
  } else {
  ...
  if (exists("fitBirdBiomassModelEvent2", envir = .GlobalEnv)) {
    simOutput <- fitBirdBiomassModelEvent2(mySim)
  } else {
