...
templateRaster <- readRDS(file.path(getPaths()$inputPath, "studyArea.rds"))[[1]]
#studyArea <- sf::as_Spatial(sf::st_as_sf(stars::st_as_stars(templateRaster), as_points = FALSE, merge = TRUE)) # requires the sf, sp, raster and stars packages
...
objects <- list("templateRaster" = templateRaster)
inputs <- list()
outputs <- list()
